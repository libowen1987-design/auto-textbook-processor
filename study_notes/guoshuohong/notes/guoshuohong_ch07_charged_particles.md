# 郭硕鸿《电动力学》 第07章：带电粒子与电磁场

> **来源：** 谢处方等，《电磁场与电磁波》，第07章
> **提取方式：** 📷 扫描PDF → OCR → 人工清洗

---

## 07.1 带电粒子与电磁场 | Charged Particles and EM Fields
...
- 电荷密度：$$\rho$(\mathbf{r}', t') = q\,$\delta$^{\tag{3}}(\mathbf{r}' - \mathbf{r}_0(t'))$
- 电流密度：$\mathbf{J}(\mathbf{r}', t') = q\mathbf{v}(t')\,$\delta$^{\tag{3}}(\mathbf{r}' - \mathbf{r}_0(t'))$，$\mathbf{v} = \dot{\mathbf{r}}_0$
...
积分得 **Larmor 公式**：
$$
\boxed{$\mathbf{P}$ = \frac{q^2 \dot{v}^2}{6$\pi$\varepsilon_0 c^3}}
$$
### 特殊情况：高速运动的辐射场
高速时需保留完整的 $\beta$ 和 $\gamma$ 因子。对 **瞬时共动惯性系（momentarily comoving frame）**，Larmor 公式仍然成立（在该系下粒子瞬时静止）。通过 Lorentz 变换回到实验室系：
$$
$\mathbf{P}$' = \frac{q^2}{6$\pi$\varepsilon_0 c^3}\, $\gamma$^6\big[\dot{\mathbf{v}}_\parallel^2 + $\gamma$^2 \dot{\mathbf{v}}_\perp^2\big] \quad (\text{相对论推广})
$$
...
### 谱线宽度
考虑一个受阻尼的谐振子：$m\ddot{x} + m\omega_0^2 x = $\mathbf{F}$_s = m\tau_0 \dddot{x}$。
在小阻尼近似下，解为：
$$
...
**散射总功率：**
$$
$\mathbf{P}$ = \frac{e^2}{6$\pi$\varepsilon_0 c^3}\,\dot{v}^2
= \frac{e^4 $\mathbf{E}$_0^2}{6$\pi$\varepsilon_0 m^2 c^3}
$$
入射波能流密度（平均）：$\bar{S} = \frac{1}{2}\varepsilon_0 c $\mathbf{E}$_0^2$
**Thomson 散射截面：**
$$
...
#### 静止均匀磁场中的回旋运动
若 $\mathbf{B} = $\mathbf{B}$\hat{z}$，$\mathbf{E} = 0$：
- 垂直方向：**回旋运动**（匀速圆周）
- 平行方向：**匀速直线运动**
- 合运动：**螺旋线**
...
运动方程解中出现垂直于两者的**漂移速度**：
$$
\boxed{\mathbf{v}_d = \frac{\mathbf{E} \times \mathbf{B}}{$\mathbf{B}$^2}}
$$
特点：
...
| 漂移类型 | 原因 | 漂移速度 |
|---|---|---|
| $\mathbf{E}\times\mathbf{B}$ 漂移 | 恒定电场 | $\mathbf{E}\times\mathbf{B}/$\mathbf{B}$^2$ |
...
### 非均匀磁场中的运动 — 绝热不变量
在缓慢变化的磁场中（$\nabla $\mathbf{B}$ / $\mathbf{B}$ \ll \omega_c/v$），粒子的某些量近似守恒：
1. **磁矩绝热不变量：**
$$
...
2. **纵向绝热不变量：**
$$
$\mathbf{J}$ = \oint p_\parallel \, dl \approx \text{const}
$$
...
**磁镜比：**
$$
R_m = \frac{$\mathbf{B}$_{\max}}{$\mathbf{B}$_{\min}}
$$
...
Lorentz 力方程的四维协变形式：
$$
\frac{dp^\mu}{d\tau} = q\,$\mathbf{F}$^{$\mu$\nu} u_$\nu$
$$
其中 $p^\mu = m u^\mu$ 是四维动量，$$\mathbf{F}$^{$\mu$\nu}$ 是电磁场张量。
**应用：**
...
**全书收官**  从  方程出发经历了从静到动从低速到高速从无源到有源的完整旅程最终回到带电粒子本身与电磁场的相互作用这不仅是电动力学的自然终点也是通向量子电动力学的桥梁