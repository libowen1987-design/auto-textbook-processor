...
> **来源：** 谢处方等，《电磁场与电磁波》，第06章
> **提取方式：** 📷 扫描PDF → OCR → 人工清洗

---

## 06.1 狭义相对论 | Special Relativity
...
### Lorentz 变换
设 $$\mathbf{S}$'$ 系沿 $S$ 系的 $x$ 轴以速度 $v$ 运动，两系原点在 $t=t'=0$ 重合：
$$
\boxed{
\begin{aligned}
x' &= \gamma (x - vt) \\
...
$$
### 间隔的分类（因果结构）
对于两个事件 $$\mathbf{P}$_1$ 和 $$\mathbf{P}$_2$，间隔 $\Delta s^2$ 不依赖于参考系：
| 类型 | $\Delta s^2$ | 物理意义 |
|------|-------------|---------|
| **类光** | $=0$ | 光信号连接 |
...
**四维电流密度**：
$$
$\mathbf{J}$^\mu = (c$\rho$, \mathbf{J}) = \rho_0 U^$\mu$
$$
...
- **Lorentz 规范条件**：$\partial_\mu $\mathbf{A}$^\mu = 0$
- **四维势**：
  $$$\mathbf{A}$^\mu = \left(\frac{\varphi}{c}, \mathbf{A}\right)$$
...
定义 **Faraday 张量**（电磁场张量）：
$$
$\mathbf{F}$^{$\mu$\nu} = \partial^\mu $\mathbf{A}$^\nu - \partial^\nu $\mathbf{A}$^\mu = \begin{pmatrix}
0 & -$\mathbf{E}$_x/c & -$\mathbf{E}$_y/c & -$\mathbf{E}$_z/c \\
$\mathbf{E}$_x/c & 0 & -$\mathbf{B}$_z & $\mathbf{B}$_y \\
$\mathbf{E}$_y/c & $\mathbf{B}$_z & 0 & -$\mathbf{B}$_x \\
$\mathbf{E}$_z/c & -$\mathbf{B}$_y & $\mathbf{B}$_x & 0
\end{pmatrix}
$$
**对偶张量**：
$$
\tilde{F}^{$\mu$\nu} = \frac{1}{2}$\varepsilon$^{$\mu$$\nu$$\rho$\sigma}$\mathbf{F}$_{$\rho$\sigma}
= \begin{pmatrix}
0 & -$\mathbf{B}$_x & -$\mathbf{B}$_y & -$\mathbf{B}$_z \\
$\mathbf{B}$_x & 0 & $\mathbf{E}$_z/c & -$\mathbf{E}$_y/c \\
$\mathbf{B}$_y & -$\mathbf{E}$_z/c & 0 & $\mathbf{E}$_x/c \\
$\mathbf{B}$_z & $\mathbf{E}$_y/c & -$\mathbf{E}$_x/c & 0
\end{pmatrix}
$$
...
### 电磁场的变换
场张量的 Lorentz 变换：$$\mathbf{F}$'^{$\mu$\nu} = \Lambda^\mu_{\;\alpha} \Lambda^\nu_{\;\beta} $\mathbf{F}$^{$\alpha$\beta}$
对于沿 $x$ 轴以速度 $v$ 运动的参考系：
$$
\boxed{
\begin{aligned}
$\mathbf{E}$'_x &= $\mathbf{E}$_x \\
$\mathbf{E}$'_y &= \gamma ($\mathbf{E}$_y - v $\mathbf{B}$_z) \\
$\mathbf{E}$'_z &= \gamma ($\mathbf{E}$_z + v $\mathbf{B}$_y)
\end{aligned}
}
\qquad
\boxed{
\begin{aligned}
$\mathbf{B}$'_x &= $\mathbf{B}$_x \\
$\mathbf{B}$'_y &= \gamma ($\mathbf{B}$_y + \frac{v}{c^2} $\mathbf{E}$_z) \\
$\mathbf{B}$'_z &= \gamma ($\mathbf{B}$_z - \frac{v}{c^2} $\mathbf{E}$_y)
\end{aligned}
}
$$
**重要认识**：电场和磁场不是独立的实体，而是电磁场张量在不同惯性系中的不同投影。
**不变量**：
- $$\mathbf{E}$ \cdot B$ 是 Lorentz 标量
- $$\mathbf{E}$^2 - c^2 $\mathbf{B}$^2$ 是 Lorentz 标量
**运动电荷的场**：
...
对于运动的点电荷，推迟势可写成四维形式：
$$
$\mathbf{A}$^$\mu$(x) = \frac{\mu_0 q}{4\pi} \frac{U^\mu}{U \cdot (x - x_q)}
$$
...
- 能量：$$\mathbf{E}$ = \gamma m_0 c^2$
**质能关系**（最重要的公式之一）：
$$
\boxed{$\mathbf{E}$ = mc^2}
$$
其中 $m = \gamma m_0$ 为相对论质量（或运动质量）。
**静能**：$$\mathbf{E}$_0 = m_0 c^2$ — 即使静止的物体也有巨大的内能
**动能**：$T = $\mathbf{E}$ - $\mathbf{E}$_0 = (\gamma - 1) m_0 c^2$
对 $v \ll c$ 展开：
$$
...
**能量-动量关系**：
$$
\boxed{$\mathbf{E}$^2 = p^2 c^2 + m_0^2 c^4}
$$
- 光子：$m_0 = 0$，$$\mathbf{E}$ = pc$
### 四维力（Minkowski 力）
$$
...
$$
\begin{aligned}
$\mathbf{F}$_x' &= $\mathbf{F}$_x \\
$\mathbf{F}$_y' &= \frac{$\mathbf{F}$_y}{$\gamma$(1 - v u_x / c^2)} \\
$\mathbf{F}$_z' &= \frac{$\mathbf{F}$_z}{$\gamma$(1 - v u_x / c^2)}
\end{aligned}
$$
### 质能等价的应用
...
| 动量 | $m_0\mathbf{u}$ | $p^\mu = ($\mathbf{E}$/c, \mathbf{p})$ |
| 力 | $\mathbf{F}$ | $K^\mu = $\gamma$(\mathbf{F}\cdot\mathbf{u}/c, \mathbf{F})$ |
| 电流 | $($\rho$, \mathbf{J})$ | $$\mathbf{J}$^\mu = (c$\rho$, \mathbf{J})$ |
| 势 | $($\varphi$, \mathbf{A})$ | $$\mathbf{A}$^\mu = ($\varphi$/c, \mathbf{A})$ |
| d'Alembert | $\nabla^2 - \partial_t^2/c^2$ | $\Box = \partial^\mu \partial_\mu$ |
| 场 | $\mathbf{E}, \mathbf{B}$ | $$\mathbf{F}$^{$\mu$\nu} = \partial^\mu $\mathbf{A}$^\nu - \partial^\nu $\mathbf{A}$^\mu$ |
| 连续方程 | $\partial_t\rho + \nabla\cdot\mathbf{J} = 0$ | $\partial_\mu $\mathbf{J}$^\mu = 0$ |
| Lorentz条件 | $\frac1{c^2}\partial_t\varphi + \nabla\cdot\mathbf{A} = 0$ | $\partial_\mu $\mathbf{A}$^\mu = 0$ |
---
## 参考
