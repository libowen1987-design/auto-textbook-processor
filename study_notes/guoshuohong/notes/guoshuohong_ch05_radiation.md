...
> **来源：** 谢处方等，《电磁场与电磁波》，第05章
> **提取方式：** 📷 扫描PDF → OCR → 人工清洗

---

## 05.1 电磁辐射 | Electromagnetic Radiation
...
$$
$$
\boxed{\mathbf{B} = \nabla \times \mathbf{A} = \frac{\mu_0 c $\mathbf{k}$^2}{4\pi} \frac{e^{ikr}}{r} (\mathbf{e}_r \times \mathbf{p}_0)}
$$
$$
\boxed{\mathbf{E} = c\mathbf{B} \times \mathbf{e}_r = \frac{1}{4\pi \varepsilon_0} $\mathbf{k}$^2 \frac{e^{ikr}}{r} (\mathbf{e}_r \times \mathbf{p}_0) \times \mathbf{e}_r}
$$
在球坐标系中，设 $\mathbf{p}_0$ 沿 $z$ 轴（$\theta = 0$ 方向）：
$$
\begin{aligned}
$\mathbf{B}$_\phi &= -\frac{\mu_0 c $\mathbf{k}$^2}{4\pi} p_0 \frac{e^{ikr}}{r} \sin\theta \\
$\mathbf{E}$_\theta &= \frac{1}{4\pi \varepsilon_0} $\mathbf{k}$^2 p_0 \frac{e^{ikr}}{r} \sin$\theta$
\end{aligned}
$$
特点：
- 横波 波$\{} \ \{} \ \{}$
...
对球面积分得 **总辐射功率**：
$$
\boxed{$\mathbf{P}$ = \frac{\mu_0 p_0^2 $\omega$^4}{12\pi c} = \frac{p_0^2 $\omega$^4}{12\pi \varepsilon_0 c^3}}
$$
即 **Larmor 公式**（电偶极辐射形式）。
对于瞬时偶极矩 $\mathbf{p}(t)$，更一般的形式：
$$
\boxed{$\mathbf{P}$ = \frac{\mu_0}{6\pi c} |\ddot{\mathbf{p}}|^2 = \frac{|\ddot{\mathbf{p}}|^2}{6\pi \varepsilon_0 c^3}}
$$
### 方向图
辐射强度随角度的分布（归一化）：
$$
...
辐射场：
$$
\mathbf{B} = \frac{\mu_0 $\mathbf{k}$^2}{4\pi} \frac{e^{ikr}}{r} (\mathbf{e}_r \times \mathbf{m}) \times \mathbf{e}_r, \quad
...
**辐射功率：**
$$
\boxed{$\mathbf{P}$_m = \frac{\mu_0 m_0^2 $\omega$^4}{12\pi c^3}}
$$
### 电四极辐射
电四极矩张量：
$$
$\mathbf{D}$_{$\alpha$\beta} = \int (3x_$\alpha$' x_$\beta$' - r'^2 \delta_{$\alpha$\beta}) $\rho$(\mathbf{r}') d^3\mathbf{r}'
$$
...
| 项 | 相对大小 | 方向图 |
|---|---|---|
| 电偶极 ($\mathbf{E}$1) | 1 | $\sin^2\theta$ |
| 磁偶极 (M1) | $\sim (d/$\lambda$)^2$ | $\sin^2\theta$ |
| 电四极 ($\mathbf{E}$2) | $\sim (d/$\lambda$)^2$ | 复杂（四瓣） |
---
## §5.4 天线辐射
### 基本天线理论
...
沿 $z$ 轴的线电流：
$$
\mathbf{I}(z,t) = I_0 \sin[$\mathbf{k}$(L/2 - |z|)] e^{-i\omega t} \mathbf{e}_z
$$
其中 $L$ 为天线长度，端电流为零。
矢势：
$$
$\mathbf{A}$_z(\mathbf{r}) = \frac{\mu_0}{4\pi} \int_{-L/2}^{L/2} \frac{I(z') e^{ik|\mathbf{r} - z'\mathbf{e}_z|}}{|\mathbf{r} - z'\mathbf{e}_z|} dz'
$$
### 半波天线 ($L = $\lambda$/2$)
电流分布：$I(z) = I_0 \cos(kz)$，$|z| \leq $\lambda$/4$
远区辐射场：
$$
$\mathbf{E}$_\theta = \frac{60 I_0}{r} \frac{\cos(\frac{\pi}{2}\cos$\theta$)}{\sin\theta} e^{i(kr - \omega t)}
$$
方向图函数（归一化）：
$$
$\mathbf{F}$($\theta$) = \frac{\cos(\frac{\pi}{2}\cos$\theta$)}{\sin\theta}
$$
**特点：**
...
方向图乘积定理：
$$
$\mathbf{F}$_{\text{array}}($\theta$, \phi) = $\mathbf{F}$_{\text{element}}($\theta$, \phi) \cdot $\mathbf{F}$_{\text{AF}}($\theta$, \phi)
$$
阵列因子（等间距 $d$，等幅，线性相位递进）：
$$
$\mathbf{F}$_{\text{AF}}($\theta$) = \frac{\sin(N$\psi$/2)}{N\sin($\psi$/2)}, \quad \psi = kd\cos\theta + $\alpha$
$$
...
3. **电偶极辐射**是最基本的辐射模式，Larmor 公式 $$\mathbf{P}$ \propto \ddot{p}^2$ 是核心结果
