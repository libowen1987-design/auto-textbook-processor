...
**传输线 (Transmission Line)**：以 TEM 导模方式传送电磁波能量或信号的导行系统，横向尺寸远小于工作波长。
...
$$Z_0 = 120\pi \cdot \frac{1}{\varepsilon_r^{1/2}} \ln\left(\frac{2D}{d}\right) \quad [D \gg d]$$
...
无耗线：$\alpha = 0$，$\beta = \omega\sqrt{LC}$
...
条件：$Z_L = Z_0$，$\Gamma_L = 0$，$\mathrm{VSWR} = 1$
...
$$P_{in} = \frac{|V_0|^2}{2Z_0} \frac{1 - e^{-2\alpha l}}{1 - |\Gamma|^2 e^{-2\alpha l}}$$

$$\eta = \frac{P_{in} - P_{out}}{P_{in}} = \frac{\text{损耗功率}}{输入功率}$$
...
**史密斯圆图用途**：
1. 已知 $\Gamma_L$，读出 VSWR、$|\Gamma|$、$\arg(\Gamma)$
...
**带宽特性**：λ/4 变换器的匹配是窄带的，工作频率偏离中心时反射增大。
...
**阶数估算**：对于阻带衰减要求 $A_s$ dB，归一化阻带频率 $\omega_s$：

$$N \approx \frac{A_s}{20\log_{10}(\omega_s)}$$

### 10.3 频率变换

- **低通 → 高通**：$\omega \to -\omega_c/\omega$
- **低通 → 带通**：$\omega \to (\omega^2 - \omega_0^2)/(\omega \cdot \Delta\omega)$
- **低通 → 带阻**：$\omega \to \omega\cdot\Delta\omega/(\omega_0^2 - \omega^2)$

### 10.4 阶跃阻抗低通滤波器

使用高阻抗线（$Z_H$）和低阻抗线（$Z_L$）交替构成，每节长度约为 $\lambda/8 \sim \lambda/4$。

- 高阻抗线：窄导体带，$Z_H = 80 \sim 120\ \Omega$
...
设计需要查耦合线设计曲线或使用全波仿真（HFSS、CST 等）精确计算。

### 10.6 滤波器设计流程

1. 确定规格：$f_0$、$FBW$、通带纹波、阻带衰减
