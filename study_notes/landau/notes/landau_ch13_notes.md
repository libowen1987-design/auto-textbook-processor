...
### 主要内容
当外加电磁场非常强（如激光）时，介质的极化与场之间不再是线性关系，产生非线性光学效应。

### 关键概念
- **非线性极化**：$\mathbf{P} = \chi^{(1)}\mathbf{E} + \chi^{(2)}\mathbf{E}\mathbf{E} + \chi^{(3)}\mathbf{E}\mathbf{E}\mathbf{E} + \ldots$
- **二次谐波产生(SHG)**：频率为$\omega$的光通过$\chi^{(2)}$介质产生$2\omega$的光
- **和频与差频产生(Sum/Difference Frequency Generation)**
- **参量放大与振荡**：利用非线性效应实现光放大
- **克尔效应(Kerr Effect)**：$\chi^{(3)}$导致折射率与光强相关
- **相位匹配**：为高效谐波产生，基波和谐波需保持相同相速度

### 应用
激光倍频器、光学参量振荡器、超连续谱产生、光开关。
