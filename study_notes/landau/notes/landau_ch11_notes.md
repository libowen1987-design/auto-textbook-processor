...
### 主要内容
当电磁场频率足够高时，介质的极化不能瞬时响应场的变化，介电常数变为频率的函数$\varepsilon(\omega)$——介电色散。

### 关键概念
- **复介电常数**：$\varepsilon(\omega) = \varepsilon'(\omega) + i\varepsilon''(\omega)$，虚部对应介质损耗
- **Drude模型**：金属中的自由电子响应，$\varepsilon(\omega) = 1 - \omega_p^2/\omega^2$（$\omega_p$为等离子体频率）
- **Lorentz模型**：束缚电子的谐振响应，$\varepsilon(\omega) = 1 + \omega_p^2/(\omega_0^2 - \omega^2 - i\gamma\omega)$
- **Kramers-Kronig关系**：$\varepsilon'(\omega)$和$\varepsilon''(\omega)$之间由因果性联系的数学关系
- **等离子体**：$\omega < \omega_p$时电磁波被反射，$\omega > \omega_p$时透射

### 应用
光学材料表征、等离子体物理、半导体光学性质、超材料设计。
