...
> 积分方程使用均匀间隔的单网格离散化。表面离散为N个点，通过求解离散化方程获得表面场。第一阶近似下矩阵元素可用小宗量Hankel函数近似。

> **第四章：随机粗糙面的散射**。研究随机粗糙面的散射问题，包括带状矩阵规范网格法（SMCG）、基于物理的双网格法（PBTG）、快速多极方法（FMM/FMM）和有序多相互作用法（MOMI）等快速计算方法，以及数值模拟与测量结果的比较。**
...
> 方法优势在于包含分支间互耦。通过假设每棵树独立散射来计算覆盖地面的行树的散射。

> 本章研究快速计算方法：SMCG、PBTG、FMM和MOMI。
