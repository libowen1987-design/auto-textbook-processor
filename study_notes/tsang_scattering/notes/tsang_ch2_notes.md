...
> **第二章：稠密介质散射**。本章研究稠密随机介质的电磁波散射，涵盖离散偶极近似（DDA）、稠密介质辐射传输理论（DMRT）、准晶近似（QCA）和相干势近似（CPA）等解析方法，以及与蒙特卡洛模拟的比较。**
...
> 该方法优势在于包含枝条间互耦。假设每棵树独立散射，计算覆盖地面的树层散射，该假设在C、L、P波段与CAA吻合良好。

> 极化率各分量由介电常数和退极化因子决定。对于圆柱形单元，考虑动态退极化修正。

> **5.2 简单树的散射**：使用DDA方法计算L系统生成的简单树的散射特性。

> **5.3 Honda树的散射**：Honda树是一种特殊的分形树结构。

> **第6节：水稻冠层散射模型**：包括模型描述和模型模拟，与ERS-1星载数据比较验证。
