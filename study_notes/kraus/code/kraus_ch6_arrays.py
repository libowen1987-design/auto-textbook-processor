...
titles = [
    'Broadside — max at 90°',
    'Endfire — max at 0°',
    'Hansen-Woodyard (~3 dB more D)',
    'Dolph-Chebyshev (equal sidelobes)',
]
for ax, (af, name, col), title_str in zip(axes.flat, data, titles):
    ax.plot(theta, af, color=col, lw=2)
    ax.fill(theta, af, alpha=0.15, color=col)
    ax.set_ylim(0, 1.3)
    ax.set_title(f'Kraus Ch6 — {name}', fontsize=10)
