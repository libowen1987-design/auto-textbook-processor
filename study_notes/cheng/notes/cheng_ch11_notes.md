# 程守洙《普通物理学》电磁学部分 第11章：习题详解

> **来源：** 谢处方等，《电磁场与电磁波》，第11章
> **提取方式：** 📷 扫描PDF → OCR → 人工清洗

---

## 11.1 习题详解 | Problem Solutions
...
$$\mathbf{E}(r) \approx j\eta_0\frac{$\mathbf{k}$ I_0 l}{4\pi r}e^{-jkr}\sin$\theta$\,\hat{\boldsymbol{\theta}} + O\left(\frac{1}{r^2}\right)$$
$$\mathbf{H}(r) \approx j\frac{$\mathbf{k}$ I_0 l}{4\pi r}e^{-jkr}\sin$\theta$\,\hat{\boldsymbol{\phi}} + O\left(\frac{1}{r^2}\right)$$
...
The radiation is **maximum in the broadside direction** ($\theta = 90^\circ$) and **null along the axis** ($\theta = 0^\circ, 180^\circ$).
...
$$\mathbf{H}(r) \approx j\frac{$\mathbf{k}$^2 $\mathbf{S}$ I}{4\pi r}e^{-jkr}\sin$\theta$\,\hat{\boldsymbol{\phi}}$$
...
| **Effective aperture $$\mathbf{A}$_e$** | $$\mathbf{A}$_e = \frac{$\lambda$^2}{4\pi}G$ (for isotropic: $$\mathbf{A}$_e = $\lambda$^2/4\pi$) |
| **Polarization** | Polarization of the radiated wave |
**Directivity** of an antenna:
$$$\mathbf{D}$ = \frac{4\pi}{\Omega_A}$$
...
**Half-power beamwidth (HPBW):** $\approx 78^\circ$ in the $\mathbf{E}$-plane.
**Directivity:** $$\mathbf{D}$ \approx 1.64$ ($2.15$ dBi).
...
For two isotropic point sources separated by distance $d$, with equal amplitude $$\mathbf{E}$_0$ and phase difference $\psi$:
**Total field:**
$$$\mathbf{E}$_T = $\mathbf{E}$_0 e^{-jkr_1} + $\mathbf{E}$_0 e^{j\psi}e^{-jkr_2} = $\mathbf{E}$_0 e^{-jkr}[1 + e^{j(kd\cos\theta + $\psi$)}]$$
**Array factor:**
$$AF = 2\cos\left(\frac{kd\cos\theta + \psi}{2}\right)$$
**Broadside array:** $\psi = 0$, $d = $\lambda$/2$ \rightarrow maximum at $\theta = 90^\circ$.
**Endfire array:** $\psi = -kd$ \rightarrow maximum at $\theta = 0^\circ$.
...
$$$\mathbf{A}$_e = \frac{$\lambda$^2}{4\pi}D$$
The **received power:**
$$$\mathbf{P}$_r = $\mathbf{A}$_e $\mathbf{S}$_{\text{inc}} = \frac{$\lambda$^2}{4\pi}$\mathbf{D}$ \cdot \frac{|$\mathbf{E}$_{\text{inc}}|^2}{\eta_0}$$
### 11-7. Some Other Antenna Types
**Traveling-wave antenna ( Beverage antenna):** $\mathbf{A}$ long wire with a matched load at the far end. Produces an endfire pattern with single-lobe main beam.
**Yagi-Uda antenna:** $\mathbf{A}$ parasitic array with one driven element, one reflector, and several directors. High gain (up to 15 dBi) with a simple structure.
...
---
## Appendix $\mathbf{A}$ — Symbols and Units
**Fundamental SI units:**
- Length: meter (m)
- Mass: kilogram (kg)
- Time: second (s)
- Current: ampere ($\mathbf{A}$)
**Derived quantities:** Force (N), Energy ($\mathbf{J}$), Power (W), Voltage (V), Resistance ($\Omega$), Capacitance ($\mathbf{F}$), Inductance ($\mathbf{H}$), Magnetic flux (Wb), Magnetic flux density (T), etc.
## Appendix $\mathbf{B}$ — Useful Material Constants
...
- Water: ~80
**Conductivities:**
- Copper: $\sigma \approx 5.8 \times 10^7$ $\mathbf{S}$/m
- Aluminum: $\sigma \approx 3.5 \times 10^7$ $\mathbf{S}$/m
- Seawater: $\sigma \approx 4$ $\mathbf{S}$/m
- Dry earth: $\sigma \approx 10^{-3}$ $\mathbf{S}$/m
