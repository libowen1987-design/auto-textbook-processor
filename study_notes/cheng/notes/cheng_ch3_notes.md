...
## 3.1 电容器与电容 | Capacitors and Capacitance
...
- Surface: $\displaystyle \mathbf{E} = \frac{1}{4$\pi$\varepsilon_0}\int_$\mathbf{S}$ \frac{\rho_s(\mathbf{r}')(\mathbf{r} - \mathbf{r}')}{|\mathbf{r} - \mathbf{r}'|^3}\,$dS$'$
...
$$\oint_$\mathbf{S}$ \mathbf{E} \cdot d\mathbf{S} = \frac{Q_{\text{enc}}}{\varepsilon_0}$$
...
$$C = \frac{Q}{V} \quad \text{(farads, $\mathbf{F}$)}$$
...
| Two parallel wires (radius $a$, separation $D$) | $C \approx \frac{$\pi$\varepsilon_0 \ell}{\ln($\mathbf{D}$/a)}$ |
**Series combination:**
...
**Energy density:**
$$w_e = \frac{1}{2}\mathbf{D} \cdot \mathbf{E} = \frac{1}{2}\varepsilon |\mathbf{E}|^2 \quad \text{($\mathbf{J}$/m}^3\text{)}$$
**Electrostatic force** on a conductor surface:
$$\mathbf{F} = \frac{1}{2}\oint_$\mathbf{S}$ \varepsilon |\mathbf{E}|^2 \hat{\mathbf{n}}\,$dS$$$
For a parallel-plate capacitor with plate area $S$ and separation $d$:
$$$\mathbf{F}$ = \frac{1}{2}\frac{\varepsilon_0 \varepsilon_r $\mathbf{S}$ $\mathbf{E}$^2}{2} = \frac{\varepsilon_0 \varepsilon_r $\mathbf{S}$ V^2}{2d^2}$$
