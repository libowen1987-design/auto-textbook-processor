...
## 6.1 物质的磁性 | Magnetic Properties of Materials
...
$\mathbf{A}$ small current loop of area $S$ carrying current $I$ has a **magnetic dipole moment**:
$$\mathbf{m} = I $\mathbf{S}$ \,\hat{\mathbf{n}}$$
...
- **Ferromagnetic materials** exhibit hysteresis — $\mathbf{B}$ vs $\mathbf{H}$ is non-linear and history-dependent.
...
- **Coercivity:** The field required to reduce $\mathbf{B}$ to zero after saturation.
### 6-10. Boundary Conditions for Magnetostatic Fields
At the interface between two media:
**For $\mathbf{B}$:**
$$\hat{\mathbf{n}} \cdot (\mathbf{B}_2 - \mathbf{B}_1) = 0 \implies $\mathbf{B}$_{2n} = $\mathbf{B}$_{1n}$$
...
$$L = \frac{N\Phi}{I} = \frac{\lambda}{I} \quad \text{(henrys, $\mathbf{H}$)}$$
...
| Parallel wires (separation $D$, radius $a$) | $L' = \frac{\mu_0}{\pi}\ln($\mathbf{D}$/a)$ |
...
$$w_m = \frac{1}{2}\mathbf{B} \cdot \mathbf{H} = \frac{|\mathbf{B}|^2}{2\mu} \quad \text{($\mathbf{J}$/m}^3\text{)}$$
