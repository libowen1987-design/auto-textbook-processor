...
**Transmission lines** are guiding structures that transmit electromagnetic power from one point to another. They are characterized by distributed parameters: series inductance $L'$ (H/m), shunt capacitance $C'$ (F/m), series resistance $R'$ ($\Omega$/m), and shunt conductance $G'$ (S/m).
...
$$L' = \frac{\mu_0 d}{w} \quad \text{(H/m)}$$
$$C' = \frac{\varepsilon_0 \varepsilon_r w}{d} \quad \text{(F/m)}$$
