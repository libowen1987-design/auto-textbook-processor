...
$$\oint_C \mathbf{E} \cdot d\boldsymbol{\ell} = -\frac{d}{dt}\int_S \mathbf{B} \cdot d\mathbf{S} \quad \text{(Faraday's law)}$$

$$\oint_C \mathbf{H} \cdot d\boldsymbol{\ell} = I_{\text{cond}} + \frac{d}{dt}\int_S \mathbf{D} \cdot d\mathbf{S} \quad \text{(Maxwell-Ampère)}$$

$$\oint_S \mathbf{D} \cdot d\mathbf{S} = Q_{\text{enc}} \quad \text{(Gauss's law for E)}$$

$$\oint_S \mathbf{B} \cdot d\mathbf{S} = 0 \quad \text{(Gauss's law for B)}$$
