...
**Volume current density** $\mathbf{J}$ (A/m$^2$) relates to the drift velocity $\mathbf{u}_d$:
...
where $\sigma$ is the **conductivity** (S/m = $\Omega^{-1}$ m$^{-1}$).
...
$$\sum_{k} I_k = 0 \quad \text{(sum of currents at a junction = 0)}$$
...
**Joule's law** for a resistor: $P = I^2 R = V^2 / R = VI$
...
$$\frac{J_{2n}}{\sigma_2} = \frac{J_{1n}}{\sigma_1}$$

### 5-7. Resistance Calculations

The resistance $R$ between two equipotential surfaces:
$$R = \frac{V}{I} = \frac{\int_\ell \mathbf{E} \cdot d\boldsymbol{\ell}}{\int_S \sigma \mathbf{E} \cdot d\mathbf{S}}$$
