...
A **plane wave** is a wave in which the field vectors $\mathbf{E}$ and $\mathbf{H}$ are constant in magnitude and direction over a plane (wavefront). Plane waves are the fundamental building blocks for analyzing more complex wave phenomena.
...
$$\mathbf{E}(z) = E_0 e^{\mp jkz} \hat{\mathbf{x}}$$
$$\mathbf{H}(z) = \pm \frac{E_0}{\eta} e^{\mp jkz} \hat{\mathbf{y}}$$

where:
- **Wave number:** $k = \omega\sqrt{\mu\varepsilon} = \frac{2\pi}{\lambda}$
...
$$\mathbf{E} = E_0(\hat{\mathbf{x}} - j\hat{\mathbf{y}})e^{-jkz}$$
...
$$\mathbf{S}_{\text{avg}} = \frac{1}{2}\text{Re}\{\tilde{\mathbf{E}} \times \tilde{\mathbf{H}}^*\} = \frac{|E_0|^2}{2\eta}\hat{\mathbf{k}} \quad \text{(for uniform plane wave)}$$

In a conducting medium:
$$\mathbf{S}_{\text{avg}} = \frac{|E_0|^2}{2\eta_{\text{real}}}\hat{\mathbf{z}}$$

### 8-5. Normal Incidence at a Plane Conducting Boundary

For a wave incident normally on a perfect conductor ($z = 0$):
- The **standing wave** pattern forms: $\mathbf{E}(z) = -jE_0\sin(kz)\hat{\mathbf{x}}$ (since $E_z=0$ at $z=0$)
- $\mathbf{H}$ has no zeros at the surface: $\mathbf{H}(z) = \frac{E_0}{\eta}\cos(kz)\hat{\mathbf{y}}$
...
$$\theta_B = \tan^{-1}\sqrt{\frac{\varepsilon_2}{\varepsilon_1}} \quad \text{(parallel polarization only)}$$
