---
title: Chapter 3: Transmission Lines and Waveguides / 第3章：传输线与波导
...
# 第3章：传输线与波导（Transmission Lines and Waveguides）
...
## §3.1 传输线理论（Transmission Line Theory）

### Distributed Parameters (p. 72, Fig. 3.1)
### 分布参数（Distributed Parameters）

A transmission line is characterized by **per-unit-length parameters**:

> 传输线的特性由单位长度的四个基本参数（R、L、G、C）决定，这些参数与低频集总元件有本质区别。
...
> 传输线方程是微波工程的核心方程，描述了电压和电流沿传输线的空间变化规律。

### Wave Equation and Solution (§3.1, pp. 73–74)
### 波动方程及其解（Wave Equation and Solution）
...
### 反射系数与电压驻波比（Reflection Coefficient and VSWR）
...
> 反射系数和VSWR是衡量传输线匹配状态的核心指标，理想匹配时 $\Gamma = 0$，VSWR = 1。

### Input Impedance of a Terminated Line (§3.1, p. 79, Eq. 3.22)
### 终端负载的输入阻抗（Input Impedance of a Terminated Line）
...
> $\lambda/4$ 线的阻抗变换特性是微波匹配网络的基石，可实现任意两个实阻抗之间的匹配。
...
### 传输线通用公式汇总（General Transmission Line Equation Summary）
...
### TEM模式场解（TEM Mode Field Solution）
...
> 同轴线是最重要的TEM模式传输线，其特性阻抗由内外导体半径比和填充介质共同决定。
...
> 同轴线的衰减来自导体损耗（趋肤效应）和介质损耗两部分，设计时需权衡填充介质与导体材料的选择。

### Higher-Order Modes (§3.2, pp. 104–106)
### 高次模式（Higher-Order Modes）
...
### 同轴线参数汇总（Coaxial Line Parameter Summary）
...
## §3.3 矩形波导（Rectangular Waveguide）
...
> 矩形波导的TE和TM模式由两个模式指数（m, n）共同决定，不同模式具有不同的场分布和截止频率。

### Dominant TE₁₀ Mode (§3.3, pp. 113–115)
### 主模式 TE₁₀（Dominant TE₁₀ Mode）
...
> TE₁₀是矩形波导的最低次模式，其截止频率仅取决于宽边尺寸a，是微波工程中最常用的波导模式。
...
> 矩形波导中相速度大于光速是波动特性，群速度小于光速且两者乘积为光速的平方——这是波导色散的本质表现。
...
### 矩形波导模式汇总（Rectangular Waveguide Mode Summary）
...
> 带状线是早期微波集成电路中广泛使用的TEM传输线，具有完整的接地平面屏蔽。
...
> 微带线的准TEM模式特性使其具有频率色散效应，有效介电常数随频率升高而增大。
...
> 微带线是现代微波集成电路（MIC）和单片微波集成电路（MMIC）的主导传输线形式，得益于其平面工艺的兼容性。

---

## §3.7 Dielectric Waveguide (pp. 137–139)
## §3.7 介质波导（Dielectric Waveguide）
...
### 光纤的联系（Optical Fiber Connection）
...
> 介质波导理论是光纤通信的理论基础，HE₁₁模式无截止频率的特性使其在光通信中尤为重要。
...
> 鳍线在毫米波频段具有重要应用，兼具波导的低损耗和微带的平面工艺优点。

---

## Summary of Transmission Line Types
## 传输线类型汇总（Summary of Transmission Line Types）
