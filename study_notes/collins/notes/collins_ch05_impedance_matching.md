---
title: Chapter 5: Impedance Transformation and Matching / 第5章：阻抗变换与匹配
source: R. E. Collin, Foundations for Microwave Engineering, 2nd Ed., §5.1–§5.16, pp. 303–370.
---

# Chapter 5: Impedance Transformation and Matching
# 第5章：阻抗变换与匹配（Impedance Transformation and Matching）
...
## §5.1 Smith圆图（Smith Chart）

### 阻抗变换的基本公式（Basic Impedance Transformation Formulas）
### Basic Impedance Transformation Formulas
...
### 反射系数的极坐标表示 (p. 305)
### Polar Representation of Reflection Coefficient
...
> Smith圆图是微波工程师最重要的图形化工具，它将阻抗/反射系数的复数关系以直观的圆图形式呈现。
...
## §5.2 电抗元件阻抗匹配（Impedance Matching with Reactive Elements）
...
### 单枝节匹配——并联枝节（Single-Stub Matching — Shunt Stub）
...
### 单枝节匹配——串联枝节（Series-Stub Matching）
...
## §5.5 集总元件匹配（Lumped-Element Matching）
...
### 电路Q值与带宽（Circuit Q and Bandwidth）
...
## §5.6 复阻抗终端的设计（Design of Complex Impedance Terminations）
...
### Design Procedure for Fig. 5.22b
...
### Design Procedure for Fig. 5.22d
...
## §5.7 阻抗失配因子的不变性（Invariant Property of Impedance Mismatch Factor）
...
### Application to Multistage Amplifiers
...
## §5.8 波导电抗元件（Waveguide Reactive Elements）

### Shunt Inductive Elements (pp. 339–341)
### 并联感性元件（Shunt Inductive Elements）
...
### 并联容性元件（Shunt Capacitive Elements）
...
## §5.9 四分之一波长变压器（Quarter-Wave Transformers）
...
## §5.10 小反射理论（Theory of Small Reflections）

### 两反射结的精确解 (图 5.34, p. 347)
### Exact Solution for Two Reflections
...
## §5.11 多节四分之一波长变压器的近似理论（Multisection Quarter-Wave Transformers）

### N 节对称变换器 (图 5.36, p. 349)
### N-Section Symmetric Transformer
...
### Cosine Series Form for Symmetric Structures
...
## §5.12 二项式变压器（Binomial Transformer）
...
### 常数 $A$ 的确定（Determination of Constant A）
...
### 反射系数与特征阻抗的关系（Reflection Coefficient vs. Impedance）
### Reflection Coefficient vs. Characteristic Impedance
...
## §5.13–5.14 Chebyshev变压器（Chebyshev Transformer）
...
### Chebyshev 变换器的反射系数（Reflection Coefficient）
...
### 与二项式变换器的对比（Comparison with Binomial Transformer）
### Comparison with Binomial Transformer
...
## §5.15 基于λ/4变压器原型的滤波器设计（Filter Design Based on QWT Prototype）
...
## §5.16 渐变传输线（Tapered Transmission Lines）
...
### 小反射近似（Small-Reflection Approximation）
### Small-Reflection Approximation
...
### 常用渐变线类型（Common Tapered Line Types）
### Common Tapered Line Types
...
### 渐变线 vs 阶梯变换器（Taper vs. Stepped Transformer）
### Taper vs. Stepped Transformer
