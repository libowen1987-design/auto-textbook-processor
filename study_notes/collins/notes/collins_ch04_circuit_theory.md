---
title: Chapter 4: Circuit Theory for Waveguiding Systems / 第4章：波导系统的电路理论
source: R. E. Collin, Field Theory of Guided Waves, 2nd Ed., Ch. 4, pp. 220–300.
---

# Chapter 4: Circuit Theory for Waveguiding Systems
# 第4章：波导系统电路理论（Circuit Theory for Waveguiding Systems）
...
## §4.1 等效电压与电流（Equivalent Voltages and Currents）
...
> 波导的电路等效描述将复杂的场解简化为电压电流形式，使得功率计算和网络分析大为简化。
...
- **Wave impedance:** For TE modes $Z_{\text{TE}} = \dfrac{\omega\mu}{\beta} = \dfrac{\eta}{\sqrt{1-(f_c/f)^2}}$
  For TM modes $Z_{\text{TE}} = \dfrac{\beta}{\omega\varepsilon} = \eta\sqrt{1-(f_c/f)^2}$
...
### 模式电压与模式电流（Mode Voltage and Mode Current）
...
## §4.2 波导元件的阻抗描述（Impedance Description of Waveguide Elements）

### One-Port (Single-Port) Circuits
### 单端口电路（One-Port Circuits）
...
### 无耗单端口终端（Lossless One-Port Termination）
...
## §4.3 Foster电抗定理（Foster's Reactance Theorem）
...
> Foster电抗定理是微波网络综合理论的基础，它确保无源无耗单端口的输入电抗随频率单调递增。

---

## §4.4 Even and Odd Properties of $Z_{\text{in}}$
## §4.4 输入阻抗的奇偶性质（Even and Odd Properties of $Z_{\text{in}}$）
...
> 互易性保证了阻抗矩阵的对称性，这是由洛伦兹互易定理在波导结上的应用所确保的。

### Lossless Junctions
### 无耗结（Lossless Junctions）
...
### 归一化阻抗/导纳矩阵（Normalized Impedance/Admittance Matrices）
...
where $z_{mn} = Z_{mn}\sqrt{\dfrac{Z_{0n}}{Z_{0m}}}$ for different reference impedances.
...
## §4.7 散射矩阵表述（Scattering Matrix Formulation）
...
> S矩阵是微波工程中最重要的参数表示方式，它具有明确的物理意义（功率波关系），且可由矢量网络分析仪直接测量。

### Two-Port S-Matrix
### 二端口S矩阵（Two-Port S-Matrix）
...
## §4.8 二端口结的散射矩阵（Scattering Matrix for a Two-Port Junction）
...
### 负载失配的影响（Effect of Output Load Mismatch）
...
### 无耗互易二端口（Lossless Reciprocal Two-Port）
...
### 示例：并联电纳（Example: Shunt Susceptance）
...
### 示例：串联电抗（Example: Series Reactance）
...
## §4.9 传输矩阵表示（Transmission-Matrix Representation）
...
### 波幅传输矩阵（Wave Amplitude Transmission Matrix）
...
### S与T矩阵的转换（Conversion Between S and T）
...
## §4.10 信号流图（Mason增益公式）（Signal Flow Graphs）
...
> 信号流图提供了分析微波网络的图形化方法，特别适用于级联和反馈系统的分析。

### Mason's Rule
### Mason法则（Mason's Rule）
...
### SFG构建规则（Rules for SFG Construction）
...
## §4.11 功率波的广义散射矩阵（Generalized Scattering Matrix for Power Waves）
...
## §4.12 波导的激励（Excitation of Waveguides）
...
## §4.13 波导的孔耦合（Aperture Coupling）

### Transverse Wall Aperture (Iris in Transverse Plane)
### 横壁孔（横向膜片）（Transverse Wall Aperture）
