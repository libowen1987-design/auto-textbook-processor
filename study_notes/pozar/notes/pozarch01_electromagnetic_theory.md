...
> **范围**: Maxwell 方程组 → 波动方程 → 平面波 → Poynting 定理 → 时谐场 → 复数介电常数 → 色散 → 反射与透射 → 例题复现
...
**量纲检查**:
- $\nabla \times \mathbf{E}$: [V/m²] = [T/s] = [Wb/m²\cdots] ✅
- $\nabla \times \mathbf{H}$: [A/m²] = [A/m²] + [C/m²\cdots] = [A/m²] ✅
...
**量纲检查**:
- $\nabla^2 \mathbf{E}$: [V/m³]
- $\mu \epsilon \partial^2 \mathbf{E} / \partial t^2$: [H/m \cdot F/m \cdot V/m \cdot 1/s²] = [V/m \cdot s²/s²] = [V/m] ✅
- $\mu \sigma \partial \mathbf{E} / \partial t$: [H/m \cdot S/m \cdot V/m \cdot 1/s] = [V/m] ✅
...
**量纲检查**:
- $\mathbf{E} \times \mathbf{H}$: [V/m \cdot A/m] = [W/m²] ✅
...
**量纲检查**:
- $\alpha$: [rad/s \cdot \sqrt(H/m \cdot F/m) \cdot (1)] = [1/m] = [Np/m] ✅
- $\beta$: [rad/s \cdot \sqrt(H/m \cdot F/m) \cdot (1)] = [1/m] = [rad/m] ✅
...
- 导致高频电阻增大（RF 电阻 \approx DC 电阻的若干倍）
...
| $\mathbf{S} = \mathbf{E} \times \mathbf{H}$ | V/m \cdot A/m = W/m² | ✅ |
| $\beta = $\omega$\sqrt{$\mu$\epsilon}$ | rad/s \cdot \sqrt(H/m\cdotF/m) = rad/m | ✅ |
| $\alpha = \sqrt{\pi f \mu \sigma}$ | \sqrt(1/s \cdot H/m \cdot S/m) = \sqrt(1/s \cdot Vs/A$\cdotm \cdot \mathbf{A}$/Vm) = 1/m | ✅ |
| $\Gamma = (\eta_2 - \eta_1)/(\eta_2 + \eta_1)$ | Ω/Ω = 无量纲 | ✅ |
| $v_p = 1/\sqrt{$\mu$\epsilon}$ | 1/\sqrt(H/m\cdotF/m) = m/s | ✅ |
...
## 1.15 原书例题复现 | Examples from Textbook

### Example 1.1 — 均匀平面波参数计算

**出处**: Pozar 4/e, p.37

**已知**:
- 工作频率 $f = 5\ \text{GHz}$
- 非磁性介质 $\mu_r = 1$ (即 $\mu = \mu_0$)
- 相对介电常数 $\epsilon_r = 2.4$
...
相速度：
$$
v_p = \frac{1}{\sqrt{\mu\epsilon}} = \frac{1}{\sqrt{\mu_0 \epsilon_0 \epsilon_r}} = \frac{c}{\sqrt{\epsilon_r}} = \frac{3 \times 10^8}{\sqrt{2.4}} \approx 1.94 \times 10^8\ \text{m/s}
$$

波长：
$$
\lambda = \frac{v_p}{f} = \frac{1.94 \times 10^8}{5 \times 10^9} \approx 0.0387\ \text{m} = 3.87\ \text{cm}
$$

相位常数：
$$
\beta = \frac{2\pi}{\lambda} = \frac{2\pi}{0.0387} \approx 162.3\ \text{rad/m}
$$

波阻抗：
$$
\eta = \sqrt{\frac{\mu}{\epsilon}} = \sqrt{\frac{\mu_0}{\epsilon_0 \epsilon_r}} = \frac{\eta_0}{\sqrt{\epsilon_r}} = \frac{377}{\sqrt{2.4}} \approx 243.3\ \Omega
$$
...
f = 5e9               # 频率 5 GHz
epsilon_r = 2.4       # 相对介电常数
mu_r = 1.0            # 相对磁导率
c = 3e8               # 真空光速
eta_0 = 376.730313    # 真空波阻抗

v_p = c / np.sqrt(epsilon_r)
lam = v_p / f
beta = 2 * np.pi / lam
eta = eta_0 / np.sqrt(epsilon_r)

print(f"v_p = {v_p:.3e} m/s")      # 1.936e8 → 1.94×10^8
print(f"λ   = {lam*100:.3f} cm")    # 3.87 cm
print(f"β   = {beta:.2f} rad/m")   # 162.3 rad/m
print(f"η   = {eta:.2f} Ω")        # 243.3 Ω
```
...
**已知**:
- 工作频率 $f = 10\ \text{GHz}$
- 电导率：Al $\sigma = 3.816 \times 10^7\ \text{S/m}$, Cu $\sigma = 5.813 \times 10^7\ \text{S/m}$, Au $\sigma = 4.098 \times 10^7\ \text{S/m}$, Ag $\sigma = 6.173 \times 10^7\ \text{S/m}$
- 磁导率 $\mu = \mu_0$ (非磁性导体)
- 良导体近似：$|\epsilon''| \gg \epsilon'$ 即 $\sigma \gg \omega \epsilon$
...
趋肤深度公式（良导体）：
$$
\delta = \frac{1}{\sqrt{\pi f \mu \sigma}} = \frac{1}{\sqrt{\pi f \mu_0 \sigma}}
$$

代入 $\mu_0 = 4\pi \times 10^{-7}\ \text{H/m}$，$f = 10^{10}\ \text{Hz}$：

$$
\delta = \frac{1}{\sqrt{\pi \times 10^{10} \times 4\pi \times 10^{-7} \times \sigma}} = \frac{1}{\sqrt{4\pi^2 \times 10^{3} \times \sigma}} = \frac{1}{2\pi \times 10^{1.5} \times \sqrt{\sigma}}
$$

| 金属 | $\sigma$ (S/m) | $\delta$ 计算值 (μm) |
|------|---------------|-------------------|
| Al  (铝)  | $3.816 \times 10^{7}$ | 0.815 |
| Cu  (铜)  | $5.813 \times 10^{7}$ | 0.660 |
| Au  (金)  | $4.098 \times 10^{7}$ | 0.786 |
| Ag  (银)  | $6.173 \times 10^{7}$ | 0.640 |

物理意义：在 10 GHz 时，电磁场穿透导体仅约 $0.6\text{–}0.8\ \mu\text{m}$，因此镀层只需极薄即可有效导电。
...
for name, sigma in metals.items():
    delta = 1 / np.sqrt(np.pi * f * mu_0 * sigma)
    print(f"{name:>2s}  δ = {delta*1e6:.3f} μm   (σ = {sigma:.3e} S/m)")
```

**输出**:
```
Al  δ = 0.815 μm   (σ = 3.816e+07 S/m)
Cu  δ = 0.660 μm   (σ = 5.813e+07 S/m)
Au  δ = 0.786 μm   (σ = 4.098e+07 S/m)
Ag  δ = 0.640 μm   (σ = 6.173e+07 S/m)
```

---

### Example 1.3 — 电流片源产生的平面波

**出处**: Pozar 4/e, p.43

**已知**:
- 在 $z = 0$ 处放置一无限大电流片，电流密度 $\mathbf{J}_s = J_{s0} \hat{x}\ \text{A/m}$（$x$ 极化）
- 电流片置于均匀无耗介质（$\epsilon, \mu$）中
- 电流片两侧空间对称

**求**: 产生的电场 $\mathbf{E}$ 和磁场 $\mathbf{H}$

**解**:

**Step 1 — 对称性与 Ansatz**

由于电流片无限大且 $x$ 极化，产生的场在 $z>0$ 区域向 $+z$ 传播，在 $z<0$ 区域向 $-z$ 传播。设：

$$
\mathbf{E}(z) =
\begin{cases}
E_0 e^{-j\beta z} \hat{x}, & z > 0 \\[4pt]
E_0 e^{+j\beta z} \hat{x}, & z < 0
\end{cases}
$$

由平面波关系 $\mathbf{H} = \frac{1}{\eta} \hat{k} \times \mathbf{E}$ 得：

$$
\mathbf{H}(z) =
\begin{cases}
\dfrac{E_0}{\eta} e^{-j\beta z} \hat{y}, & z > 0 \\[10pt]
-\dfrac{E_0}{\eta} e^{+j\beta z} \hat{y}, & z < 0
\end{cases}
$$

其中 $\beta = \omega\sqrt{\mu\epsilon}$，$\eta = \sqrt{\mu/\epsilon}$。
...
$$
\hat{n} \times (\mathbf{H}_+ - \mathbf{H}_-) = \mathbf{J}_s
$$

其中 $\hat{n} = \hat{z}$ 指向 $z>0$ 区域。代入 $\mathbf{H}$：

$$
\hat{z} \times \left( \frac{E_0}{\eta} \hat{y} - \left(-\frac{E_0}{\eta} \hat{y}\right) \right) = J_{s0} \hat{x}
$$

$$
\hat{z} \times \left( \frac{2E_0}{\eta} \hat{y} \right) = \frac{2E_0}{\eta} \hat{x} = J_{s0} \hat{x}
$$

解得振幅：

$$
E_0 = -\frac{J_{s0}}{2}\, \eta
$$
...
\mathbf{E}(z) =
\begin{cases}
-\dfrac{J_{s0}}{2}\, \eta \, e^{-j\beta z} \hat{x}, & z > 0 \\[10pt]
-\dfrac{J_{s0}}{2}\, \eta \, e^{+j\beta z} \hat{x}, & z < 0
\end{cases}
\;\Longrightarrow\;
E_x(z) = -\dfrac{J_{s0}}{2}\, \eta \, e^{-j\beta|z|}
$$
...
\mathbf{H}(z) =
\begin{cases}
-\dfrac{J_{s0}}{2} \, e^{-j\beta z} \hat{y}, & z > 0 \\[10pt]
+\dfrac{J_{s0}}{2} \, e^{+j\beta z} \hat{y}, & z < 0
\end{cases}
\;\Longrightarrow\;
H_y(z) = \pm \dfrac{J_{s0}}{2} \, e^{-j\beta|z|}
$$

其中 $\pm$ 分别对应 $z>0$（取 $-$）和 $z<0$（取 $+$）。

**物理意义**:
- 电流片两侧均产生平面波，振幅对称
- 电场幅值 $|E_x| = \dfrac{|J_{s0}|}{2} \eta$
- 磁场幅值 $|H_y| = \dfrac{|J_{s0}|}{2}$
- 磁场在电流片两侧方向相反（符合安培环路定理）
...
epsilon_0 = 8.854e-12
epsilon_r = 2.4     # 示例介质
eta = np.sqrt(mu_0 / (epsilon_0 * epsilon_r))

# 计算场幅值
E_amp = -J_s0 / 2 * eta   # V/m
H_amp_pos = -J_s0 / 2      # z>0, A/m
H_amp_neg = +J_s0 / 2      # z<0, A/m

print(f"η          = {eta:.2f} Ω")
print(f"E_x amplitude   = {E_amp:.4f} V/m")
print(f"H_y amplitude   = {H_amp_pos:.4f} A/m (z>0)")
print(f"H_y amplitude   = {H_amp_neg:.4f} A/m (z<0)")
print()
print("电场 E_x(z) = -η J_s0/2 · exp(-jβ|z|)")
print("磁场 H_y(z) = ± J_s0/2 · exp(-jβ|z|)")
```

**输出**:
```
η          = 243.18 Ω
E_x amplitude   = -121.5905 V/m
H_y amplitude   = -0.5000 A/m (z>0)
H_y amplitude   = +0.5000 A/m (z<0)

电场 E_x(z) = -η J_s0/2 · exp(-jβ|z|)
磁场 H_y(z) = ± J_s0/2 · exp(-jβ|z|)
```
