...
- Q 因子本质上衡量"能量存储效率"：每周期储存 2\piQ 倍的能量比损耗的多
- 串联谐振在谐振点阻抗最小（短路特性）；并联谐振在谐振点阻抗最大（开路特性）
- 高 Q → 窄带宽 → 高选频性
- 耦合系数 \beta 控制匹配状态，在滤波器设计中是关键的调谐参数
...
对比：$\lambda$/4 短路谐振器的 Q 与 $\lambda$/2 短路谐振器形式相同（$$\beta$/2\alpha$），但系数 $l$ 不同。
...
谐振条件与短路 $\lambda$/2 相同：$l = n$\lambda$/2$（$n = 1, 2, \ldots$）

**无载 Q 表达式与短路 $\lambda$/2 相同。**
...
- $\lambda$/2 短路谐振器：两端短路 → 两端为电流波腹、电压波节 → 谐振时中间为电压波腹
- $\lambda$/4 短路谐振器：一端短路（电流波腹），一端开路（电压波腹）→ 类似四分之一波长驻波
- 开路 $\lambda$/2 谐振器常用于微带线滤波器设计
- Q 值受 \alpha 控制：低损耗线 → 高 Q；高 \alpha → 低 Q
...
矩形波导腔（长 a \times 宽 b \times 高 d）是最经典的微波腔体。
...
这是将介质柱近似为磁壁圆柱腔的 TM 模，但实际介质谐振器常用 TE_{01\delta} 模。

**更精确的近似（Itoh-Rudokas 公式，用于 TE_{01\delta} 模，$\varepsilon_r$ 较大时）：**
...
量纲验证：$34\ \text{(mm\cdotGHz)} / (a\ \text{[mm]}) \cdot 1 = \text{GHz}$ ✓
...
- 介质谐振器是微波滤波器小型化的关键技术
- TE_{01\delta} 模的场图类似矩形腔 TE_{101} — 磁场环形封闭，电场沿轴向
...
\approx 1.759\times 10^{11}\ \text{rad/(s\cdotT)}
$$

实用单位：$$\gamma$/(2$\pi$) \approx 28\ \text{GHz/T} = 2.8\ \text{MHz/Gauss}$

量纲：$[$\gamma$] = \text{rad/(s\cdotT)}$ ✓
...
| $\lambda$/2 谐振 Q | $Q_0 = $\beta$/(2$\alpha$)$ | (6.24) |
...
| S11 ↔ \beta | $S_{11}(\omega_0) = (1-$\beta$)/(1+$\beta$)$ | (6.87) |
