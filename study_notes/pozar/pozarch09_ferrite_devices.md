...
铁氧体 (ferrite) 是铁磁绝缘体，化学式 $\text{MO·Fe}_2\text{O}_3$（M = Mn, Mg, Ni, Zn, Y 等）。
其关键特性：
...
**量纲检查** ✅:
- $[\gamma] = \text{C/kg} = \text{A·s/kg}$，$[\mu_0 \mathbf{H}] = \text{T} = \text{kg/(A·s²)}$
- 右侧: $(\text{C/kg})(\text{A/m})(\text{kg/(A·s²)}) = (\text{A/(m·s)}) = [d\mathbf{M}/dt]$ ✅
...
**量纲检查** ✅:
- $\gamma\mu_0 H_0$: $(\text{C/kg})(\text{H/m})(\text{A/m}) = (\text{C/kg})(\text{kg/C·s}) = 1/\text{s}$ ✅
- $f_0$ 单位为 Hz ✅
...
```
┌─────────────────────────────────────────────────────┐
│ 1. 选材: f, Ms, ΔH, εr (YIG → 窄带高Q, NiFe → 宽带)│
├─────────────────────────────────────────────────────┤
│ 2. 计算 ω0 = γμ0H0, ωm = γμ0Ms                     │
├─────────────────────────────────────────────────────┤
│ 3. 计算 μ, κ (含损耗: 用复数 ω0 + jαω)             │
├─────────────────────────────────────────────────────┤
│ 4. 确定 μ+, μ- (圆极化本征模)                      │
├─────────────────────────────────────────────────────┤
│ 5. 器件特定设计:                                    │
