...
1
2πRC p , as the frequency increases, the impedance of
...
1
2π√LlC , where the capacitive reactance of the
...
and the required 5, ±12 V voltage inside the equipment, and performs the same power
...
switchingsignalisapproximately1/πtr.Anincreaseoftr canreducetheHFharmonic
dBµV
...
standard points in the CM and DM spectrum, respectively, in dBμV. (VLimits)d B is
the speciﬁed limit values, also in dBμV. The 6 dB is added to make the attenuation
...
cies, the abscissa is Hz, and the ordinate is dBμV, as shown in Fig. 7.24. Taking the
...
signal is about 1 μs. The rise time of the CMOS 5 V logic device is 100 ns, which not
...
For a +5 V logic input power supply, a 0.01 μf capacitor is generally sufﬁcient.
...
the 1553B chip transceiver input supply. A low ESR/ESI capacitor of 2.2 μf should
be sufﬁcient.
...
mended to place a 10 μf external tantalum capacitor and a 0.1μf capacitor as close
...
supply. In addition, it is recommended to place a 0.1 μf capacitor as close as possible
...
According to the transmission rate, it is recommended to use 0.1 μF and 1000 pf
capacitors.
...
much as possible. Use a 360° loop connection for the shield grounding.
...
According to the interface requirements, the rising and falling edge is 0.2 μs
minimum, and the clock’s minimum rising edge is 6 μs. The actual measured rising
and falling edges on each signal line are around 1us. When the dielectric constant εr
...
the low-frequency end. The EMC technical requirements are 45–60 dBμV/m in the
...
should be placed in a large ground area, the maximum spacing of the vias is λ/10. In
order to prevent process problems such as vias overlapping, it is required that the via
spacing is no less than λ/60, and the speciﬁc value should be determined according
...
frequency of the capacitor and the inductor 1/√L * C, the resistor exhibits a minimum
...
N = W/πr is applicable to rectangular holes of near-ﬁeld magnetic ﬁeld, N =
j5.79 × 10−5 × f × D is applicable to round holes of near-ﬁeld electric ﬁeld,
N = j6.69 × 10−5 × f × W is applicable to rectangular holes of near-ﬁeld electric
ﬁeld,
r is the distance from the source to the shield (cm), and
f is the EM wave frequency (MHz).
(3) Multiple reﬂection loss Ba
When Aa ≥10 dB, it can be ignored.
When Aa < 10 dB,
Ba = 20 lg

1 −(N −1)2
...
SE1 = A + R + B
(7.10.8)
For aluminum, μr = 1, σr = 0.61, f = 2 × 109 Hz, and t = 1 mm.
...
the RF signal ≥2 GHz is analyzed. The data in Table 7.6 does not take into account
...
μ0, ε0 are, respectively, the magnetic permeability and dielectric constant of the
air inside the housing, μ0 = 4π × 10−7H/m, ε0 = 8.85 × 10−12F/m.
Then,
...
working temperature range of −60 to +120 °C and can be cured at room temperature.
...
Estimation of the target impedance of the PCB is Z = Vin · 5%
Imax
(7.11.1)
