...
59
\nabla\times ⃗E(⃗r, t) = −\partial
\partialt
⃗B(⃗r, t) (Faraday’s law of EM induction),
(4.2.2)
\nabla\cdot ⃗D(⃗r, t) = $\rho$(⃗r, t) (Gauss’ law for electric ﬁeld),
(4.2.3)
\nabla\cdot ⃗B(⃗r, t) = 0 (Gauss’ law for magnetic ﬁeld),
(4.2.4)
where
⃗E, ⃗B, ⃗H, ⃗D, ⃗J, and \rho are the real variables of position and time;
...
$\rho$(⃗r, t) is the charge density (C/m3).
...
\nabla\times ⃗H(⃗r) = j\omega ⃗D(⃗r) + ⃗J(⃗r) = j$\omega$$\varepsilon$

1 + $\sigma$
$\omega$

⃗E,
(4.2.5)
\nabla\times ⃗E(⃗r) = −j\omega ⃗B(⃗r),
(4.2.6)
\nabla\cdot ⃗D(⃗r) = $\rho$(⃗r),
(4.2.7)
\nabla\cdot ⃗B(⃗r) = 0.
(4.2.8)
...
\rhos is the surface charge density on the interface (C/m2).
...
terer or radiator is large (generally deﬁned as greater than 10$\lambda$). Correspondingly,
...
= \etaD,
(4.4.2)
where \eta is the efﬁciency, namely, the ratio of the radiated power to the input power.
...
according to a 10\lambda distance; if the antenna is an aperture type, and the aperture is
D, then it is divided according to a distance of 2D/$\lambda$. The transmission of EM wave
...
PT is the transmitter power, GT is the transmitter antenna gain, and \lambda is the wave
wavelength.
...
and inversely proportional to the distance of the antenna’s electrical wavelength R/$\lambda$.
...
wavelength$\lambda$)maybethefrequencyontheworkingfrequencybandofthetransmitter,
...
• if |f s – f r| < 0.2 \times f r there is a fundamental interference margin;
• if 0.1 \times f s < 10 \times f r there is a transmitter interference margin;
• if 0.1 \times f s < f r < 10 \times f s there is a receiver interference margin;
• if 0.1 \times f s < 10 \times f r or 0.1 \times f r < 10 \times f s there is a clutter interference margin,
...
 fm = |\pma \times fs1 \pm b \times fs2 \pm \cdot \cdot \cdot \pm n \times fsN|
Bm = |a \times Bs1 + b \times Bs2 + \cdot \cdot \cdot + n \times BsN| ,
(4.5.5)
where
...
 fc = |\pma \times fs \pm b \times fLO|
Bc = |$\pma \times \mathbf{B}$s \pm b \times BLO| ,
(4.5.8)
where
