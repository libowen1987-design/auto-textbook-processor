...
not only the platform passband, but also the LNA bandwidth \pm10%.
e.
...
(ACS). The magnetic moment of small spacecraft should not exceed 1A \cdot
m2, and that of large spacecraft should not exceed 10A \cdot m2. The solar array
needs separate control. If a low-orbit spacecraft has a magnetometer, the
impact of the platform on the deﬂection should be less than 1\muT.
...
,
(5.3.2)
where \eta0 is the impedance of free space wave, and its value is 120$\pi$.
...
PR =
E2
120$\pi \times \mathbf{G}$ R \times \lambda2
4$\pi$
(5.3.5)
...
antenna pattern
2 tan 0.5◦\approx2 tan 1◦\approx0.035
(5.3.6)
B. TT&C station test
...
R($\tau$) cos ϕm,
(5.3.7)
where \tau is the delay between local signal and direct signal; R($\tau$) is the autocorrelation
function of pseudo-random code sequence; d is the interval between the leading
correlator and the lagging correlator; \delta is the delay between multipath signal and
direct signal; \alpha is the amplitude ratio between multipath signal and direct signal (i.e.,
...
between \delta and \tau is obtained. The multipath error curve can be obtained by further
...
Yd = −10 lg

cos2 \beta + sin2 \varphi \cdot 10−Dp(ϕb)

10 + sin2 \beta \cdot 10−Dpsat

10
dB,
(5.3.8)
...
Yu = −10 lg

cos2 \beta + sin2 \beta \cdot 10−Dp(\psib)

10 + sin2 \beta \cdot 10−Dpst

10
dB,
(5.3.9)
where \psib is the angle between the main radiation direction and the radiation direction
of the ground station; Dp(\psib) is the expected polar decoupling of the satellite;


112
5
Analysis of Spacecraft System-Level Electromagnetic Compatibility
Dp(\psib) = S//(\psib) −S+(\psib) (dB); S// and S+ are the expected direction pattern
...
same formula.
In the above analysis, it is necessary to calculate the relative orientation angle $\beta$.
\beta is calculated as
\beta = |\varepsilon1 −\varepsilon2| + $\delta$,
(5.3.11)
where \varepsilon1 and \varepsilon2 represent the polarization angle of expectation and interference
signals, respectively; and \delta is the error.
5.4
...
external equipment of the spacecraft in the position ($\theta$, ϕ) of the transmitting antenna
...
AT T ,
(5.5.6b)
where |E($\theta$, ϕ)| is the peak electric ﬁeld intensity at the external equipment of the
spacecraft, in V/m; E is the effective value of electric ﬁeld strength at the external
device of the spacecraft, in V/m; F($\theta$, ϕ) is the normalized directivity coefﬁcient
...
Pr( f, t) \cdot 240k2
F2($\theta$, ϕ) \cdot Gr \cdot \gamma \cdot cos2 $\xi$

AT T
(5.5.8b)
...
F($\theta$, ϕ) is the normalization direction coefﬁcient of the receiving antenna in the
direction ($\theta$, ϕ) of the incoming wave from the radiation electric ﬁeld; Gr is the
main lobe gain of the receiving antenna; \gamma is the antenna port matching coefﬁcient;
cos2 \xi is the polarization matching coefﬁcient; \xi is the angle of polarization, in ^\circ; k
is wave number, in 1/m; \lambda is the wavelength, in m.
...
5
Analysis of Spacecraft System-Level Electromagnetic Compatibility
where,S isthesensitivitythresholdofsensitiveequipment,indB\muV/m;E istheinten-
sity of the interference electric ﬁeld, in dB\muV/m; SM is electromagnetic interference
margin, in dB.
...
=
(4$\pi$$\beta$
s/h)(2m)1/2, \beta is the correction factor,
\beta = 1 −
1
8(ϕ)2
 s2
...
(5.6.3)
ϕ = ϕ1 + (
ϕ −eV )(x/s) −1.15\lambdas2/x(s −x)
(5.6.4)
In Eq. (5.6.4),
\lambda = e2 ln 2

(8$\pi$\varepsilon0\varepsilonrs)
...
\varepsilonr—Relative dielectric constant;
\varepsilon0—Dielectric constant of free space;
...
V = V1 cos(\omega1t) + V2 cos(\omega2t)
(5.6.6)
Assuming that the radius r of the metal wire is 50 \mum, and the contact area
approximates to \pir2, the current can be calculated. Using XFDTD based on the
...
z) and i(z + 
z). Since 
z ≪$\lambda$, for the uniform transmission
line, the transmission line equation is 
z ≪$\lambda$, for the uniform transmission line,
...

U(z) = A1e−j\betaz + A2e j\betaz
I(z) =
1
Z0

A1e−j\betaz + A2e j\betaz ,
(5.7.3)
where \beta = $\omega$
\sqrt
LC and Z0 =
...

U(z) = U0 cos \betaz −j Z0I0 sin \betaz
I(z) = I0 cos \betaz −j U0
Z0 sin \betaz
(5.7.4)
...
line and a disturbed cable as the receiving line. The line length is l > $\lambda$. Set up
the coordinate system, where the coordinate is from x = 0 to x = l. At x = 0, an
excitation voltage ug = Us sin \omegat is applied between the transmitting line and the
reference conductor. The conductor is surrounded by a no-loss uniform medium with
a dielectric constant of \varepsilon and a magnetic permeability of $\mu$. The terminal impedances
...
dUG(x)
dx
= −j$\omega$[LG IG(x) + L M IR(x)]
dUR(x)
dx
= −j$\omega$[L M IG(x) + L R IR(x)]
dIG(x)
dx
= −j$\omega$[(CG + CM)UG(x) −CMUR(x)]
dIR(x)
dx
= −j$\omega$[(CR + CM)UR(x) −CMUG(x)]
.
(5.7.6)
...
\tauG and \tauR are the time constants of transmitting and receiving circuits, respectively.
The above is the analysis of cable coupling in high frequency. In the case of low
frequency, the length of the cable is far less than the wavelength, that is, l ≪$\lambda$, then
	UR(l) = U L
R (l) + U C
R (l)
UR(0) = U L
...
length l and the wavelength of the cable is $\lambda$, that is, h ≪l and h ≪$\lambda$, a cable can be
...
impedance, and wavelength of the cable. If the length of the cable is less than $\lambda$

10,
...
R2 + 2\pi f L2,
(5.8.4)
where f is the frequency; B is magnetic ﬂux density; l is the length of the cable; $\lambda$
...
Axax + Ayay + Azaz

e−s(\kappax x+\kappay y+\kappazz),
(5.8.7)
where, \kappa is the propagation vector. The distribution source of MTLs is
 V D(z, s)
I D(z, s)

= Γ (s)e−s\kappazz,
...
0n (G + sC)

\cdot \cdot \cdot 2\kappax Axxi −2\kappay Azyi \cdot \cdot \cdot | \cdot \cdot \cdot 2Axxi + 2Ayyi \cdot \cdot \cdot
T
(5.8.8)
Let
M = −Q(s) −s\kappaz I2n
...
eMl −I2n

M−1Γ (s) = −e Q(s)l M−1Γ (s) + e−s\kappaz I2n M−1Γ (s)
(5.8.10)
so, we have
 V(l, s)
I(l, s)
...
,
(5.8.12)
where RL representstheterminalload;\psiDnV and\psiDf V areequivalentvoltagesources
of near ﬁeld and far ﬁeld, respectively; \psiDnI and \psiDf I are equivalent current sources
...
of 7.23 m. There are two wave transmission windows of size 0.3 m \times 0.3 m on the
...
located at the side of the cable with an angle of 45^\circ to the ground plane. The induced
