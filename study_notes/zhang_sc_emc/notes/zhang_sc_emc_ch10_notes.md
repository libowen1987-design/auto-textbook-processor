...
(10.1.1)
Assume that the values on either side of the equation are $\beta$, then
\beta = (xc −xa)/(xb −xa), \beta = (yc −ya)/(yb −ya)
(10.1.2)
yc = ya + $\beta$(yb −ya) = (1 −$\beta$)ya + \betayb
(10.1.3)
...
\beta = (0.8 −0.5)/(1 −0.5) = 0.6
(10.1.4)
yc = 1.1 + 0.6 \times (1.3 −1.1) = 1.22(dB)
(10.1.5)
...
is within \pm3 dB of the measured value of oscilloscope.
...
I = V + F + CL
(10.3.1)
where I is the interference current on the power lead in dB\muA;
V is the reading on the receiver in dB\muV;
...
I (dB\muA) = V (dB\muV) + S[dBS or dB(1/)]
(10.3.2)
or I (dB\muA) = V (dB\muV) −R(dB)
(10.3.3)
...
receiver’s
reading
Calculate the current value dB\muA
Receiver readings on the calibration 
frequency dB\muV + probe factor +
cable loss
...
to verify that the measurement is within \pm3 dB of the injected signal level. The
correction factors include the 20 dB attenuator, the insertion loss of the 0.25 \muF
...
voltage at the 0.25 \muF capacitor port of the LISN and estimates the interference
Signal output port
Power supply
50Ω load or the
50Ω input port of
...
UR = Uc + F + CL + 20
(10.3.4)
where UR is the interference voltage on the power line in dB\muV;
Uc is the reading of the receiver in dB\muV;
...
the receiver to avoid overload or damage to the receiver when the 0.25 \muF capacitor
...
EUT through the coupling transformer. A 10 \muF feedthrough capacitor is parallel-
...
by an oscilloscope or a voltmeter. The 10 \muF feedthrough capacitor forms a short
...
Convert the reading into dB\muV
Equal?
Calculate the limit at 
...
I = U + (−34) + CL + A
(10.3.5)
where I is the induced current on the cable under test in dB\muA;
U is the receiver reading in dB\muV;
CL is the cable loss in dB;
...
current value is 109 dB\muA, and the output voltage of the calibration ﬁxture is 109 +
34 = 143 (dB\muV). If the calibration is monitored by the receiver, an external 36 dB
attenuator is required to ensure that the maximum input power of the receiver port
does not exceed 0 dBm, or 107 dB\muV. Therefore, an attenuator of 40 dB or above is
...
that the measured value of the test system is within \pm3 dB of the injected signal
level.
...
U is the receiver reading in dB\muV;
AF is the magnetic ﬁeld antenna correction factor in dBpT/\muV;
CL is the cable loss in dB.
10.3.5.2
...
antenna should be used, with a typical opening dimension of 69.0 cm \times 94.5 cm.
At the range of 1–18 GHz, a double-ridged horn antenna with a typical opening
dimension of 24.2 cm \times 13.6 cm is used.
The test method is
...
that the measured value is within \pm3 dB of the injected signal level.
...
E = V + CL2 −GLNA + CL1 + AF
(10.3.7)
where E is the electric ﬁeld strength radiated by the EUT in dB\muV/m;
V is the receiver’s reading in dB\muV;
...
temperature;
k is the Boltzmann constant, k = 1.38 \times 10−23 J/K;
...
according to the law of 10 \times lg(B), when B = 10 kHz: N = −174 dBm + 10 \times lg(10
\times 103) = −134 dBm.
...
When So = Si \times G, then
N F = (Si/Ni)/(Si \times G/No) = N0/Ni \times G
(10.3.11)
...
(10.3.21)
where 107 is the conversion from dBm to dB\muV, then
...
considering all losses, make sure that the measured value is within \pm3 dB of the
...
as required below:
(1) At 10 kHz to 200 MHz, when D \leq3 m, the antenna is placed on the centerline
...
25 \muH inductor are not clearly deﬁned, so different test results may be obtained. The
...
V is the output level of the signal generator B obtained from this step in dB\muV;
V 20 is the output level of the signal generator B obtained from step (1) in dB\muV.
...
of the signal generator B until f 0 \pm f IF (where f IF is the intermediate frequency
...
ﬁxture, that is:
the current value is converted into voltage as U = I \times R = 250 (V).
...
response
20
f BW = 1.0 \times f knee
f BW = 1.0 \times f knee
10
f BW = 1.3 \times f knee
f BW = 1.2 \times f knee
3
f BW = 1.9 \times f knee
f BW = 1.4 \times f knee
...
is written as
y = f (x1, x2, x3, . . . , xN) + \deltaz1 + \deltaz2 + \deltaz3 + \cdot \cdot \cdot + \deltazN
(10.5.3)
...
is \pm2.0 dB, the estimated correction value is taken as 0, and has a uniform distribution
...
tion value u(\deltaVpa)
The tolerance limit of the receiver pulse amplitude response given in CISPR16-4-2
is \pm1.5 dB, the estimation of the correction value is taken as 0, and has a uniform
...
3. Uncertainty of the receiver noise ﬂoor inﬂuence correction value u(\deltaVnf)
...
limit of the receiver noise ﬂoor proximity is \pm0.5 dB, the estimation of the correction
...
considered to be \pm1.5 dB with a rectangular distribution, the standard uncertainty
...
parameters are known, it is impossible to calculate \deltaM, but it can be determined by
the above formula that the estimated value of \deltaM does not exceed the limit value
range \deltaM\pm, which is
\deltaM\pm = 20 lg
...
| = 20 lg |

1 \pm 0.94 \times 0.09 \times 0.812
| = 20 lg |(1 \pm 0.056)|
(10.5.17)
then, \deltaM+ = 0.47 dB, \deltaM = −0.50 dB.
The probability distribution of \deltaM approximates an inverse sine distribution (U-
shaped distribution), and its half-width is no greater than (\deltaM+ −\deltaM−)/2, the
standard uncertainty is half-width divided by
\sqrt
2. Thus, the standard uncertainty
introduced by mismatch is
u(\deltaM) = \deltaM+ −\deltaM−
2
\sqrt
2
= 0.34 dB
(10.5.18)
To simplify the calculation, \deltaM\pm = 20 lg(1 \pm |Γ e||Γ r||S21|2) can be expressed as
...

1 \pm |Γe∥Γr∥S21|2
= 20 ln(1 \pm |Γe||Γr||S21|2)
ln10
(10.5.19)
Since |Γ e||Γ r||S21|2 ≪1, if performing Taylor expansion to the natural logarithm
in the above formula, then: ln(1 \pm |Γ e||Γ r||S21|2) \approx\pm |Γ e||Γ r||S21|2 and
20
ln10 = 8.68,
that is
\deltaM\pm = \pm8.68|Γe∥Γr∥S21|2
(10.5.20)
...
2 = 6.14|Γe∥Γr∥S21|2
(10.5.21)
Substitute |Γ e| = 0.94, |Γ r| = 0.09, S21| = 0.81 into the above equation, then u(\deltaM)
...
0.33. Substitute |Γ e| = 0.94, |Γ r| = 0.33, S21| = 0.81 into u(\deltaM) = 6.14|Γ e||Γ r||S21|2,
then u(\deltaM) = 1.25 dB. It can be seen that the mismatch has a large effect on the
...
55.44, 55.21, 55.32, 55.48, 55.25 dB\muV, the arithmetic mean is 55.34 dB\muV, and the
...
10.5 Measurement Uncertainty Analysis of Electronic Equipment EMC Test
453
is used as the measurement result, the standard uncertainty is u = 0.10 dB/\sqrtn, =
0.10 dB/
\sqrt
10 = 0.03 dB.
10.5.2
...
30
0.004
(4) For large current loads, the CE101 measurement uses a LISN of 5 \muH, the
...
the 0.25 \muF voltage division coefﬁcient of the LISN.
(3) When calibrating the CE102 test system, the measurement values of the receiver
are required to be within \pm3 dB of the injected signal level at 10 kHz, 100 kHz,
...
(8) Foralargecurrentload,theCE102measurementusesa5\muHLISN,themeasure-
...
the voltage drop across the source impedance, a 10 \muF capacitor should be
...
generator is 0–400 V, the peak value of the reverse voltage (V s) is \leq30% \times V p,
the rise time (tr) is 1.5 \mus \pm 0.5 \mus, fall time (tf) is (3.5 \pm 0.5) \mus, pulse width
(td) is 5.0 (1 \pm 22%) \mus, reverse pulse width (ts) is \leq20 \mus, PRF is 3–10 Hz;
MIL-STD-461D: 1993 REQUIREMENTS FOR THE CONTROL OF ELEC-
TROMAGNETIC INTERFERENCE EMISSIONS AND SUSCEPTIBILITY
requires that the pulse width of the peak signal generator is 0.15, 5, 10 \mus, and
...
10 A, the attenuator will have a level as high as 500 V (10 A \times 50 ) and
...
the ESD. Make sure that the ambient temperature is between 15 and 35 ^\circC and
...
signal bandwidth or the range of \pm5% fundamental frequency (whichever is
...
dB\muV/m, which requires the test system to have a low background noise ﬂoor.
...
(1) When the measurement bandwidth is \geq50 MHz, the peak-to-peak value is
\leq1.4 V;
(2) The spike duration ts \leq5 \mus.
...
(1) <111 dB\muA, at switching frequency;
(2) <99 dB\muA, at frequencies other than the switching frequency.
...
The test limit requirement is current change rate dI/dt \leq10 A/\mus.
...
Load transients CE test requirement is: dI/dt \geq1 A/\mus. During the load transient
change, the bus voltage change rate requirement is: dV/dt \leq1 V/\mus.
In addition to the bus voltage change rate requirements, the bus voltage change
requirements are shown in Fig. 10.61: when the load transient is greater than 400 \mus
but not exceeding 4.5 ms, the bus voltage does not exceed the average bus voltage
\pm3 V; beyond 4.5 ms, the bus voltage does not exceed the average bus voltage \pm0.3 V;
at any time, the bus voltage does not exceed the average bus voltage \pm3.7 V.
...
\omega2
(10.6.2)
where \omegap is the frequency of the plume;
...
reduce the internal pressure of the product to below 1.5 \times 10−3 Pa before an RF
...
(1) The environmental vacuum in the vacuum tank should be lower than 1.5 \times 10−3
Pa.
(2) The vacuum in the critical area of the product RF path should also be lower than
1.5 \times 10−3 Pa. Therefore, the environmental vacuum should be maintained for
...
materials, the test vacuum can be reduced to 1.3 \times 10−2 Pa, but should be
...
➀Radiative \beta source: it can provide high-energy electrons, which can pass
