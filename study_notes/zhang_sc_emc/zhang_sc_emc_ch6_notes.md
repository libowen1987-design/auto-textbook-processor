...
an operating frequency below 100 kHz and signal sensitivity level below 1 μV,
as shown in Fig. 6.2. The maximum interference of 120 dB μA can be applied
...
(c)
A spike signal of 10 μs width, 8 V or 16A.
EUT
Oscilloscope
Signal 
source
...
of the spike signal is 10 μs and E = 200 V. The length of the sensing cable is
...
transmission signal bandwidth or ±5%
...
signal bandwidth or ±5% frequency
...
bandwidth or ±5% frequency range of
...
transmitter under test (transmitting state) or within ±5% of its fundamental frequency
...
(a)
Receiver: 34 dB μV;
(b) Transmitter and ampliﬁer (standby state): 34 dB μV;
(c)
...
1.1 V (120 dB μV). Based on equipment-level EMC requirements (such as MIL-
STD-461), the required CE102 emission limit at 30 kHz is 90 dB μV, which means
...
a signal width of 3.3 μs, a rise/fall time of 100 ns, and a frequency of interest of
600 kHz.
...
20 log(100 V) + 120 −16.0 dB = 144.0 dB μV
(6.3.4)
...
144.0 dB μV −65.0 dB −6 dB + 20 log(50 ) = 107.0 dB μV
(6.3.6)
...
Vi(dB μV) = 20 × log(Ii(mA)) + 20 × log(Zi) + 60
(6.3.7)
...
limit V f at 100 kHz is approximately 80 dB μV. The suppression requirement of the
...
Y = 118.4 dB μV −80 dB μV + 6 dB = 44.4 dB
(6.3.9)
...
voltage, or 50 V, for a duration of 10 μs. Assuming the line resistance is 0.9 , then
50 V = 100 V ×
...
(6.3.10)
The resulted capacitance value is 16 μF.
...
the connector and the cable should be 360° shielded, thus avoiding “Pigtail” type
grounding. 360° shielding coverage means there are no gaps or exposed areas
...
mitter’s fundamental transmit signal bandwidth or the ±5% frequency range of the
...
width or ±5% of the fundamental frequency (whichever is greater). The starting
...
equipment casing at this frequency is SE, the required limit value is L [dB μV/m],
...
frequency is 35 dB μV/m, and there is also a special band requirement of 1.5 GHz
with a limit value of 10 dB μV/m. Due to the presence of a 4-way circuit of the local
...
through which the cable enters the housing. The basic principle is to design a 360°
...
This test typically uses a voltage with a contact discharge of ±5 kV and an air
discharge of ±10 kV. During the test, the EUT shall not exhibit any faults, perfor-
...
the range of VSS ≤V i ≤VDD, and the excess input terminals are not suspended,
...
200 × 10−12 = 1.59(GHz)
(6.4.1)
According to the slope variation, the amplitude is about 65.2 dB μV at 1350 MHz;
and about 60.1 dB μV at 1.59 GHz.
...
m

= V (dB μV) + 2π f C(dBS) + 20 × log(0.013 × l × 2h) + 40 log( fMHz)
=
...
50 MHz. The emission limits for these two bands are typically 15 and 25 dB μV/m.
...
ence plane is 1 mm, the shielding effect is ≥30 dB at a frequency of 100 kHz.
...
excitation frequency or harmonic wavelength, i.e. λ/20. For example, a λ/20 of a
...
than λ/20, using an additional metal foil connection or the like structure can also
...
of the wiring, use a 45° angle or rounded wiring instead, as shown in Fig. 6.49.
...
Generally, the outside of the chamfer can be cut into a 45° bevel angle, as shown in
...
the EM wave, and generally, the limit is less than λ/20.
...
(1) A/V > 1.7 × 10−6/mm, where A is the total area of the vent and V is the volume
of the equipment;
...
have good shielding effects, which can provide a good 360° loop grounding for the
...
≤10 m, minimum contact area >1 cm2
...
combine to form a characteristic impedance Z0, where Z0 = √L/C. Whether be the
...
Shielded cables must form a 360° shielding cover over its length, and should also
include the connector back-shells at both ends of the cable. The connector must form
a 360° bonding with the back-shells.
6.5
Summary
