...
for ax, Z, name in zip(axes, [(Z_sc,'SC'), (Z_oc,'OC')],
                       ["Short-circuited stub", "Open-circuited stub"]):
    ax.plot(Z.real/Z0, Z.imag/Z0, 'b-', lw=1.5)
    ax.set_xlim(-3, 3); ax.set_ylim(-3, 3)
    ax.axhline(0, color='k', lw=0.5); ax.axvline(0, color='k', lw=0.5)
    ax.set_xlabel("Re(Z)/Z0"); ax.set_ylabel("Im(Z)/Z0")
    ax.set_aspect('equal'); ax.grid(True, alpha=0.3)
    ax.set_title(f"Kraus Ch9 — {name} Stub Impedance\nL=0 to λ (Sec 9-3)", fontsize=10)
...
L, W, eps_eff = patch_design(2.45, 2.2, 1.59)
